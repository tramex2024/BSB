...
function displayOrders(newOrders, tab) {
    ...
    // Keep ordersToRemove logic as is
    ...
    // --- SIMPLIFIED FINAL MESSAGE LOGIC ---
    if (newOrders.length === 0) {
        ...
        currentDisplayedOrders.clear(); // Ensure map is clear if no orders
    } else {
        // Only update innerHTML if it doesn't already contain orders, or if it's the initial load message
        if (orderListDiv.innerHTML.includes('Loading orders') || orderListDiv.innerHTML.includes('No orders found')) {
             // Clear only the "Loading" or "No orders" message if actual orders are present
             orderListDiv.innerHTML = '';
        }
        // Log success only if there were actual orders to display
...
async function getBalances() {
    ...
    try {
        ...
        // --- Start of essential change for getBalances ---
        // Expecting an array directly from the backend, as per your logs.
        ...
        // --- End of essential change for getBalances ---
...
async function fetchOpenOrdersData() {
    ...
    try {
        ...
        // --- The fix for open orders ---
        // Backend's raw response from BitMart has `data.orders`, but your backend appears to simplify this.
        // Assuming your backend sends an object like `{ orders: [...] }` as per previous context
        // If your backend directly sends an array for open-orders as well, then remove `.orders`
...
async function fetchOrders(tab) {
    ...
    try {
        if (tab === 'opened') {
            const response = await fetchFromBackend(`/api/user/bitmart/open-orders?symbol=${TRADE_SYMBOL}`);
            // Assuming open-orders also returns an array of orders directly or an object with an 'orders' array.
            // Based on the first set of logs, `response.orders` was being accessed, so keeping that.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('tab-opened')) {
        setActiveTab('tab-opened'); // This internally calls fetchOrders
    }
