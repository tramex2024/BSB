document.addEventListener('DOMContentLoaded', () => {
    ...
    authForm.addEventListener('submit', async (e) => {
        ...
        if (!token) { // First step: request token
            try {
                const response = await fetch('http://localhost:3000/api/auth/request-token', { // Replace with your Render URL
                    method: 'POST',
            ...
            try {
                const response = await fetch('http://localhost:3000/api/auth/verify-token', { // Replace with your Render URL
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
