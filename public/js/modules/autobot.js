...
export async function initializeAutobotView() {
    ...
    if (socket) {
        ...
        // 🛑 NUEVO LISTENER: Captura la sincronización completa del estado (incluye lcoverage/lnorder)
        socket.on('full-state-sync', (data) => {
             console.log('[Socket.io] Sincronización completa recibida para Autobot.');
            const botState = data.botState;
            
            // 1. Actualizar la UI de estado (RUNNING/STOPPED y Configuración)
            updateBotUI(botState);
            
            // 2. Actualizar las métricas dinámicas (lcoverage, lnorder, ppc)
