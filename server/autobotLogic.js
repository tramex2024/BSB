        ...
        switch (botStateObj.state) {
            case 'RUNNING':
                ...
                if (botStateObj.ac > 0) {
                    ...
                    // --- ESTA ES LA LÍNEA CLAVE PARA LOS LOGS DEL ANALIZADOR EN EL FRONTEND ---
                    // Dependiendo del tipo de señal, el mensaje tendrá un color diferente en el frontend
            ...
            case 'BUYING':
                ...
                if (botStateObj.ac > 0) {
                    ...
                    emitLogMessage(botStateObj.userId, `DEBUG_COVERAGE: Próximo monto USDT: ${nextUSDTAmount.toFixed(2)}, Precio de última orden: ${lastOrderPrice.toFixed(2)}, Precio para próxima cobertura: ${nextCoveragePrice.toFixed(2)} USDT.`, 'debug');
                    ...
                    emitLogMessage(botStateObj.userId, `Esperando confirmación de la primera orden o actualización de AC (puede que la primera orden esté pendiente).`, 'info');
            ...
            case 'NO_COVERAGE':
                emitLogMessage(botStateObj.userId, `Estado: **NO_COVERAGE**. Esperando fondos para la próxima orden de ${botStateObj.nextCoverageUSDTAmount.toFixed(2)} USDT @ ${botStateObj.nextCoverageTargetPrice.toFixed(2)}.`, 'warning');
...
async function startBotStrategy(userId, botParams, bitmartCreds) {
    ...
    if (botState.ac === 0) {
        ...
        emitLogMessage(userId, `Reanudando bot con Activo Comprado (AC) existente: ${botState.ac.toFixed(8)} ${BASE_CURRENCY}. Estado: ${botState.state}`, 'warning');
    ...
    const intervalId = setInterval(async () => {
        ...
        if (latestBotState.isRunning && latestBotState.state !== 'STOPPED' && latestBotState.state !== 'ERROR') {
            ...
            emitLogMessage(userId, `El bot no está en estado RUNNING/BUYING/SELLING. **Deteniendo ciclo de ejecución**.`, 'warning');
