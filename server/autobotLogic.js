...
async function botCycle(priceFromWebSocket) {
    try {
        ...
        // Asignación de fondos según la configuración (usamos los campos correctos para la validación)
        // NOTA: Asumo que 'purchaseUsdt' y 'purchaseBTC' son los montos asignados
        const assignedUSDT = parseFloat(botState.config.long.purchaseUsdt || 0);
        const assignedBTC = parseFloat(botState.config.short.sellBtc || 0); // Corregido a sellBtc para Short
        ...
        if (assignedUSDT > availableUSDT && botState.lstate !== 'STOPPED') {
            ...
            await updateBotState('STOPPED', 'long'); // Usar la función para la actualización
        ...
        const dependencies = {
            ...
            // --- CORRECCIÓN CRÍTICA DE INYECCIÓN ---
            config: botState.config, // ⬅️ Inyectamos la configuración
            creds: {                 // ⬅️ Inyectamos las credenciales requeridas por orderManager
            ...
            updateBotState, 
            updateLStateData,
            updateGeneralBotState // Inyectamos la función genérica de actualización
        };
        ...
        if (botState.sstate !== 'STOPPED') {
            // await runShortStrategy(); // Dejada como comentario, como en tu código original
        }
...
async function updateLStateData(lStateData) {
    try {
        // En un bot que maneja ambas estrategias, idealmente usaríamos un campo sStateData, 
        // pero asumimos que lStateData maneja los datos de ciclo para ambas temporalmente si no hay sStateData.
