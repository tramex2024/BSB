// BSB/server/autobotLogic.js (RESTAURADO con LBalance/SBalance y Corregido)
...
async function updateBotState(newState, strategy) {
    try {
        ...
        // CORRECCIÓN CRÍTICA DE SINCRONIZACIÓN: Emitimos el estado completo.
...
/**
 * Función genérica para actualizar campos top-level en el modelo Autobot (usado para LBalance/SBalance).
...
async function botCycle(priceFromWebSocket) {
    try {
        ...
        // ⚠️ Nota: Las validaciones críticas por falta de fondos reales (STOPPED) siguen comentadas.
        // El control de presupuesto (LBalance/SBalance) se maneja en los estados LRunning/LBuying.
        ...
        const dependencies = {
            ...
            updateBotState, 
            updateLStateData, // RESTAURADO para guardar PPC, AC, etc.
            updateGeneralBotState // RESTAURADO para guardar LBalance/SBalance
        };
