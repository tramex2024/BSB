...
async function placeCoverageBuyOrder(botStateObj, bitmartCreds) {
    ...
    if (targetPrice === undefined || targetPrice === null || targetPrice === 0) {
        console.error(`[AUTOBOT][${botStateObj.userId}] Precio objetivo de cobertura no disponible o es cero.`);
        return; // Do not proceed with order if targetPrice is invalid
    }

    let sizeBTC = sizeUSDT / targetPrice;
    // Ensure sizeBTC is a valid number before toFixed
    if (isNaN(sizeBTC) || !isFinite(sizeBTC)) {
        console.error(`[AUTOBOT][${botStateObj.userId}] Error calculando sizeBTC para cobertura. sizeUSDT: ${sizeUSDT}, targetPrice: ${targetPrice}`);
        return; // Do not proceed with order if sizeBTC is invalid
    }
...
async function runBotLogic(botStateObj, bitmartCreds) {
    ...
    try {
        ...
        // If the error is due to invalid credentials, attempt to stop the bot and notify
        if (error.message.includes('Error interno del servidor al obtener y desencriptar credenciales de BitMart') || error.message.includes('API keys not configured')) {
            ...
            // Ensure botStateObj is defined before attempting to change its state
            if (botStateObj) {
                botStateObj.state = 'STOPPED';
                botStateObj.isRunning = false; // Also set isRunning to false
                await saveBotState(botStateObj);
            ...
            console.error(`[AUTOBOT][${botStateObj.userId}] Error crítico de tipo de dato (toFixed o similar). Deteniendo el bot.`);
            if (botStateObj) {
                botStateObj.state = 'ERROR'; // Set to ERROR state for manual intervention
