...
function determineEntryPoint(candlesWithIndicators, currentPrice, symbol = SYMBOL) {
    if (!candlesWithIndicators || candlesWithIndicators.length < 2) { // Necesitamos al menos 2 velas con RSI para un cruce
    ...
    // `lastCandle` es la última vela completa y `prevCandle` es la anterior a esa.
    let prevCandleRSI = candlesWithIndicators[candlesWithIndicators.length - 2].rsi;
    let lastCandleRSI = candlesWithIndicators[candlesWithIndicators.length - 1].rsi;

    // AHORA, para el análisis en tiempo real, calculamos un "RSI actual" usando el currentPrice.
    ...
    allClosePricesIncludingCurrent.push(currentPrice);

    const latestRsiValues = RSI.calculate({ values: allClosePricesIncludingCurrent, period: RSI_PERIOD });
    const currentRSI = latestRsiValues[latestRsiValues.length - 1]; // Este es el RSI más actual considerando currentPrice

    if (isNaN(prevCandleRSI) || isNaN(lastCandleRSI) || isNaN(currentRSI)) {
        const result = { action: "HOLD", symbol: symbol, reason: `RSI no calculado o inválido para las últimas velas o precio actual. Prev RSI: ${prevCandleRSI}, Last Candle RSI: ${lastCandleRSI}, Current Price RSI: ${currentRSI}.` };
        console.log(`[ANALYZER-SEÑAL] ${result.action} - ${result.reason}`);
        return result;
    }

    // LOG TEMPORAL: Mostrar valores del RSI
    console.log(`[ANALYZER-DEBUG] Analizando señales - RSI Vela Anterior Completa: ${prevCandleRSI.toFixed(2)}, RSI Última Vela Completa: ${lastCandleRSI.toFixed(2)}, RSI (con Precio Actual): ${currentRSI.toFixed(2)}`);
    ...
    // Usamos el RSI de la ÚLTIMA VELA COMPLETA (lastCandleRSI) como 'prev' para el cruce,
    // y el RSI calculado con el PRECIO ACTUAL (currentRSI) como 'current'.
    if (lastCandleRSI <= RSI_OVERSOLD && currentRSI > RSI_OVERSOLD) {
        buySignalDetected = true;
        buyReason.push(`RSI (con precio actual) cruzó ${RSI_OVERSOLD} al alza desde la última vela completa (${lastCandleRSI.toFixed(2)} -> ${currentRSI.toFixed(2)})`);
    ...
    else if (currentRSI < RSI_OVERSOLD && currentRSI > lastCandleRSI) { // currentRSI está en sobreventa Y sube respecto al RSI de la última vela cerrada
        buySignalDetected = true;
        buyReason.push(`RSI (con precio actual) en sobreventa (${currentRSI.toFixed(2)}) y subiendo desde el RSI de la última vela cerrada (${lastCandleRSI.toFixed(2)})`);
    ...
    if (lastCandleRSI >= RSI_OVERBOUGHT && currentRSI < RSI_OVERBOUGHT) {
        sellSignalDetected = true;
        sellReason.push(`RSI (con precio actual) cruzó ${RSI_OVERBOUGHT} a la baja desde la última vela completa (${lastCandleRSI.toFixed(2)} -> ${currentRSI.toFixed(2)})`);
    ...
    else if (currentRSI > RSI_OVERBOUGHT && currentRSI < lastCandleRSI) { // currentRSI está en sobrecompra Y baja respecto al RSI de la última vela cerrada
        sellSignalDetected = true;
        sellReason.push(`RSI (con precio actual) en sobrecompra (${currentRSI.toFixed(2)}) y bajando desde el RSI de la última vela cerrada (${lastCandleRSI.toFixed(2)})`);
