...
async function updateBotConfig(req, res) {
    try {
        ...
        const amtAI = newConfig.ai?.amountUsdt || (botState.config.ai ? botState.config.ai.amountUsdt : 0);

        // 2. Calcular Blindaje Automático (40% de cobertura)
        const shield = processUserInputs(amtL, amtS, amtAI);

        // 3. Mezcla segura para campos manuales (profit_percent, etc.)
        ...
        const update = {
            ...
            // AI (Aseguramos que si no existe el objeto en DB, no explote)
'config.ai.amountUsdt': parseFloat(amtAI) || 0,
...
'config.ai.enabled': newConfig.ai?.enabled !== undefined ? 
    newConfig.ai.enabled : (botState.config.ai?.enabled || false)
        ...
        // 5. Sincronizar con el motor en tiempo real
        if (updatedBot) {
        ...
        return res.status(500).json({ success: false, message: "Error interno en el servidor de blindaje." });
    }
}
