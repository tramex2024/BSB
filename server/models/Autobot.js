 ...
 * Corregido para coincidir con el modelo de Mongoose (profit_percent)
 */
function syncConfigWithBackend() {
    ...
    // Estructuramos el payload exactamente como lo espera tu modelo Autobot.js
    const payload = {
        config: {
            long: {
    ...
    // Emitimos el evento que el servidor escucha para actualizar la DB
...
export async function initializeAutobotView() {
    ...
    // 1. Iniciamos los escuchas de los inputs
    setupConfigListeners();

    // 2. Iniciamos el gráfico (con delay para asegurar que el DOM esté listo)
    setTimeout(() => {
    ...
    // 3. Configuración de los botones Start/Stop
    ...
    // 5. Pedir el estado inicial al conectar
    if (socket && socket.connected) {
