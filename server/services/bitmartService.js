// backend/services/bitmartService.js
...
async function makeRequest(method, path, paramsOrData = {}, isPrivate = true, authCredentials = {}) {
    ...
    try {
        ...
        if (response.data.message === 'success' || response.data.code === 1000 || response.data.message === 'OK') {
        ...
        if (error.response) {
            ...
            throw new Error('No se recibió respuesta de BitMart API. Posible problema de red o firewall o la API no está disponible.');
        } else {
...
async function placeOrder(authCredentials, symbol, side, type, size, price) {
    ...
    if (type === 'limit') {
        ...
        if (side === 'buy') { requestBody.notional = size.toString(); }
        else if (side === 'sell') { requestBody.size = size.toString(); }
 ...
 * @param {string} symbol - Par de trading.
 * @param {string} interval - Intervalo de las velas.
