...
// --- Nueva Función para Verificar Depósitos ---
async function getDepositHistory(apiCredentials, currency = null, page = 1, limit = 50) {
    const { apiKey, apiSecret, memo } = apiCredentials; // Memo is typically not used for general queries
    const PATH = '/cloud/spot/v1/wallet/deposit-withdraw-history';
    const timestamp = Date.now().toString();
    const body = {
        side: 'deposit', // We are interested in deposits
        // No order_id needed for history
        currency: currency, // Optional: filter by currency
        page_no: page,
        page_size: limit,
        // start_time: (Date.now() - 30 * 24 * 60 * 60 * 1000).toString(), // Last 30 days, or a more relevant range
        // end_time: Date.now().toString()
    };
    const query = new URLSearchParams(body).toString(); // For query string parameters

    const sign = crypto.createHmac('sha256', apiSecret).update(`${timestamp}#${PATH}#${query}`).digest('hex');
    ...
    try {
        ...
        if (response.data && response.data.code === 1000) {
            return response.data.data.records; // Array of deposit records
        } else {
            console.error('BitMart API Error (Deposit History):', response.data);
            throw new Error(`BitMart API error: ${response.data.message || 'Unknown error'}`);
        }
    } catch (error) {
        console.error('Error fetching deposit history from BitMart:', error.message);
