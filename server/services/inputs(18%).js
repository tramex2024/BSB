...
function processUserInputs(amtL, amtS, amtAI, existingConfig = {}) {
    ...
    const calculateScalpingGrid = (totalAmount, side) => {
        ...
        // 1. DETERMINAR NÚMERO DE NIVELES Y ORDENES DINÁMICAS
let n = 0;
let cumulativeBase = 0;
let orderBase = PURCHASE_FIXED;
let currentMultiplier = 1.9; // Empezamos suave

let levelsConfig = []; // Para guardar el size de cada nivel

while (cumulativeBase + orderBase <= totalAmount && n < MAX_LEVELS) {
    levelsConfig.push(orderBase);
    cumulativeBase += orderBase;
    n++;
    
    // Aumentamos el multiplicador gradualmente de 1.9 hacia 2.1
