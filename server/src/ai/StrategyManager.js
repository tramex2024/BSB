...
/**
 * STRATEGY MANAGER - IA ANALYTICAL CORE
 * 2026 Refinement: Triple Confluence Filter
 */
...
class StrategyManager {
    static calculate(history) {
        // Minimum history increased to 200 to accommodate Institutional EMA
        ...
        try {
            // 1. ADX - Trend Strength (Is it worth entering?)
            ...
            // 2. Stochastic RSI - Momentum (Is the timing right?)
            ...
            // --- SCORE ENGINE (WEIGHTING SYSTEM) ---
            let score = 0;

            // Criteria A: Macro Trend (40%)
            if (isAboveInstitutional) {
                score += 30; 
                if (isBullishCross) score += 10;
            } else {
                score -= 30; // Strictly avoid buying under EMA 200
            }

            // Criteria B: Momentum (40%)
            if (latestStoch && prevStoch) {
                const kDiff = latestStoch.k - prevStoch.k;
                // Oversold with upward turn
                if (latestStoch.k < 25 && kDiff > 3) score += 40;
                // Overbought (Severe penalty for chasing)
                else if (latestStoch.k > 80) score -= 50;
                // Medium bullish impulse
                else if (kDiff > 5) score += 15;
            }

            // Criteria C: ADX Volatility (20%)
            if (latestADX > 25) score += 20; 
            else if (latestADX < 15) score -= 40; // Sideways range = Danger
            ...
            console.error("❌ Math Error in StrategyManager:", e);
            return null;
        }
    }

    static _generateMessage(bullish, adx, stoch, conf) {
        if (conf >= 0.85) return "🚀 HIGH CONFIDENCE: Full technical alignment.";
        if (stoch && stoch.k > 80) return "⚠️ EXHAUSTION: Price is overextended.";
        if (adx < 18) return "😴 LOW VOLUME: Market is currently dormant.";
        if (!bullish) return "📉 MACRO FILTER: Bearish trend dominant.";
        return "🔍 SCANNING: Looking for optimal confluence...";
    }
}
