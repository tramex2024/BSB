/**
 * STRATEGY MANAGER - NÚCLEO ANALÍTICO IA
 * Refinado 2026: Filtro de Confluencia Triple
 */
...
class StrategyManager {
    static calculate(history) {
        // Incrementamos el historial mínimo para incluir la EMA de 200 para mayor seguridad
        ...
        try {
            // 1. ADX - Fuerza (¿Vale la pena entrar?)
            ...
            // 2. Stochastic RSI - Momentum (¿Es el momento justo?)
            ...
            // 3. EMAs - Estructura (Filtro de marea)
            const ema9 = EMA.calculate({ period: 9, values: closeValues });
            const ema21 = EMA.calculate({ period: 21, values: closeValues });
            const ema200 = EMA.calculate({ period: 200, values: closeValues }); // 👈 Filtro Institucional
            ...
            // --- SCORE ENGINE (SISTEMA DE PESOS) ---
            let score = 0;

            // Criterio A: Tendencia Macro (40%)
            if (isAboveInstitutional) {
                score += 30; 
                if (isBullishCross) score += 10;
            } else {
                score -= 30; // Evitamos compras bajo la EMA 200
            }

            // Criterio B: Momentum (40%)
            if (latestStoch && prevStoch) {
                const kDiff = latestStoch.k - prevStoch.k;
                // Sobreventa con giro al alza
                if (latestStoch.k < 25 && kDiff > 3) score += 40;
                // Sobrecompra (Penalización severa)
                else if (latestStoch.k > 80) score -= 50;
                // Impulso alcista medio
                else if (kDiff > 5) score += 15;
            }

            // Criterio C: Volatilidad ADX (20%)
            if (latestADX > 25) score += 20; 
            else if (latestADX < 15) score -= 40; // Rango lateral = Peligro
    ...
    static _generateMessage(bullish, adx, stoch, conf) {
        ...
        if (stoch && stoch.k > 80) return "⚠️ AGOTAMIENTO: El precio está muy extendido.";
        if (adx < 18) return "😴 SIN VOLUMEN: El mercado está dormido.";
