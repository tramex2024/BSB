...
async function handleSuccessfulBuy(botState, orderDetails, log) {
    ...
    const isFirstOrder = botState.lStateData.orderCountInCycle === 0; // ✅ CORRECCIÓN 1: Definir si es la primera orden

    // 🛑 CORRECCIÓN 1: Si es la primera orden, inicializar a 0 para evitar residuos de ciclos anteriores.
    const currentTotalQty = isFirstOrder ? 0 : parseFloat(botState.lStateData.ac || 0);    
    const currentAI = isFirstOrder ? 0 : parseFloat(botState.lStateData.ai || 0); // 🛑 INVERSIÓN ACUMULADA (CON FEES)
    ...
    // 2.3. ✅ CORRECCIÓN 2: Calcular el Precio de Venta (LTPrice)
