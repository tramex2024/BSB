...
            
            // LLAMADA AJUSTADA: Usamos la configuración, log, y los updaters directamente de las dependencias.
...
        // ⚠️ BLOQUE DE RECUPERACIÓN AUTÓNOMA (Sustituye 'ERROR')
        // Si falla la persistencia DEBE haber un problema crítico con la DB.
        // La mejor manera de recuperarse es limpiar lastOrder (si existe) y mantener el estado.
        log(`CRITICAL PERSISTENCE ERROR: Falló el reseteo del estado tras venta exitosa/asumida. Causa: ${error.message}`, 'error');
        ...
        try {
            // Intenta limpiar lastOrder para que el proceso no se quede bloqueado con una orden fantasma.
            await updateLStateData({ 'lastOrder': null });
        } catch (dbError) {
             // Si incluso la limpieza falla, registra un error grave.
             log(`FALLA DE RECUPERACIÓN: No se pudo limpiar lastOrder. Revise la conexión/estado de la DB.`, 'error');
        }
        
        // El estado se mantiene en SELLING (o el estado actual) para que el RunHandler lo reevalúe.
    }
}
