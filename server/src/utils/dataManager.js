...
 * (PPC, AC, lastExecutionPrice), y ahora también 'ai' (Amount Invested).
 *
 * @param {object} botState - Estado actual del bot.
 * @param {object} orderDetails - Detalles de la orden llenada.
...
        // Usar findOneAndUpdate con $set para limpiar lastOrder
...
    // --- 3. GESTIÓN DEL CAPITAL INVERTIDO (ai) ---

    // Obtener el monto de inversión actual (ai)
    const currentAmountInvested = parseFloat(botState.lStateData.ai || 0);
    // Sumar el costo real de la orden llenada
    const newAmountInvested = currentAmountInvested + actualUsdtSpent;
    const newAI = newAmountInvested; // Usamos newAI para mayor claridad

    // --- 4. GESTIÓN DEL CAPITAL RESTANTE (LBalance y Refund) ---
...
    }
    
    // --- 5. ACTUALIZACIÓN ATÓMICA DE ESTADO EN LA BASE DE DATOS (CRÍTICO) ---
...
            'lStateData.ai': newAI, // ✅ CORRECCIÓN: Usamos el campo 'ai' para Amount Invested
...
    };
    
    log(`[AUDITORÍA 1/3] -> ANTES de la actualización atómica. PPC: ${newPPC.toFixed(2)}, AC: ${newTotalQty.toFixed(8)}, AI: ${newAI.toFixed(2)}`, 'debug');
...
        log(`[AUDITORÍA 3/3] -> VERIFICACIÓN EN DB. PPC leído: ${updatedBot.lStateData.ppc.toFixed(2)}, AC leído: ${updatedBot.lStateData.ac.toFixed(8)}, AI leído: ${updatedBot.lStateData.ai.toFixed(2)}, LState: ${updatedBot.lstate}`, 'debug');
    } else {
        log('[AUDITORÍA 2/3 y 3/3] -> ERROR: No se encontró el documento de Autobot después de la actualización.', 'error');
        return;
    }

    log(`[LONG] Orden confirmada. Nuevo PPC: ${newPPC.toFixed(2)}, Qty Total (AC): ${newTotalQty.toFixed(8)}. AI: ${newAI.toFixed(2)}. Transicionando a BUYING.`, 'success');
}
...
            "ppc": 0, 
            "ac": 0, 
            "ai": 0, // <-- Corregido el nombre a 'ai' en la inicialización
            "ppv": 0, 
            "av": 0, 
            "orderCountInCycle": 0, 
            "lastOrder": null, 
